...
public class KhachHangDAO implements DAOinterface<KhachHang> {
	...
	public ArrayList<KhachHang> selectAll() {
		...
		try {
			...
			while (rs.next()) {
				...
				boolean dangKyNhanBangTin = rs.getBoolean("dangkinhantin");
	...
	public int insert(KhachHang t) {
		...
		try {
			...
			// Bước 2: tạo ra đối tượng statement
			String sql = "INSERT INTO khachhang (makhachhang, tendangnhap, matkhau, hovaten, gioitinh, diachi, diachinhanhang, diachimuahang, ngaysinh, sodienthoai, email, dangkinhantin) "
					+ " VALUES (?,?,?,?,?,?,?,?,?,?,?,?)";
	...
	public int update(KhachHang t) {
		...
		try {
			...
			String sql = "UPDATE khachhang " + " SET " + " tendangnhap=?" + ", matkhau=?" + ", hovaten=?" + ", gioitinh=?"
					...
					+ ", email=?" + ", dangkinhantin=?" + " WHERE makhachhang=?";
	...
	public KhachHang selectTenDangNhapAndPassWord(KhachHang t) {
		KhachHang ketQua=null;
		try {
			// Bước 1: tạo kết nối đến CSDL
			Connection con = JDBCUtil.getConnection();
